...
- **Three invariants in `sw.js`, each of which fixed a bug where the page rendered but the PDF silently never appeared. Don't quietly revert them:**
  ...
  - **Every JS chunk is required at install, and `/` (the offline navigation fallback); everything else is best-effort per URL.** Per-URL tolerance exists because failing the whole install on the first bad response left the visitor with no offline shell at all and re-downloaded the whole site on every subsequent visit, and it still covers HTML pages and other assets, which come back on the next online visit. It does **not** cover the JS, and the comment that used to say a miss "still resolves over the network on demand" was false: Vercel serves only the current deployment, so every content-hashed URL from the previous one 404s the moment a deploy lands (measured 2026-09-21 against production - three old hashes 404, three current 200), and roughly half of each tool page's chunk URLs change per deploy. Paired with no `skipWaiting()`, a cache that activated with a gap keeps serving until every page from its build closes and cannot refill that gap from the network in the meantime. Reproduced end to end: evict one chunk, publish the next build, and `/sign/` paints its static shell perfectly while `astro-island` never hydrates - a bare 404, nothing thrown, FCP/LCP/CLS all healthy while the tool is dead. Requiring the JS costs no extra bytes (install already fetches those URLs) and only changes what a miss does: the install fails, this worker never activates, the previous one keeps serving its own complete cache, and the visitor stays on a working older build. Required URLs get three attempts with linear backoff so one dropped response on a weak connection does not cost the new build. Requests go through a small concurrency pool.
