...
- 2026-09-13, evening (P1 from the UX review, thumbnails at one page per ~500 ms): the cost was per file, not per page. `getDocument()` booted a brand-new pdf.js Web Worker for every file (200-400 ms each, the main thread idle meanwhile); `src/lib/thumbnails.js` now shares one `PDFWorker` across every document it opens, which per-document `destroy()` never tears down. Measured on Chromium against dev with the reviewer's fixtures (17 pages): first image at 254 ms, then one every ~17 ms, all done by ~550 ms; the 375 draft-restore first row went from ~1600 ms to ~485 ms. Guard: e2e/merge/merge-thumbnail-throughput.spec.js.
