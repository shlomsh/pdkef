...
Re-homed from `sign-tool-architecture` to `fonts-and-script-support` on the board cleanup: every other
Sign architecture ticket is done, and this one is a shaping-guard concern of the same kind as FONT-01,
so it belongs with the font work it would be run against. Scope unchanged.
