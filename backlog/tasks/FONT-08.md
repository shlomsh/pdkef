...
- **Advance parity** (`e2e/sign/greek-font-parity.spec.js`, Hebrew-Guard-A style: fontkit's per-segment shaped advance vs. this browser's own `measureText`), ten Greek names/phrases with spaces: **10/10 passed, 0.000px disagreement on every case.**
...
- **Separate, unrelated finding surfaced while building the parity guard, flagged for follow-up rather than fixed here:** shaping `'Νικόλαος Δημητρίου'` (pure Greek, no Mynerve involved) through the *existing* upright faces Arimo and Tinos disagrees with the browser by ~1.77px (113 font units) on both - Mynerve shapes the identical string at 0.000px. `e2e/sign/greek-font-parity.spec.js`'s module doc records the measurement; the guard itself is scoped to Mynerve only so this pre-existing, previously-unguarded gap in the three already-bundled upright faces doesn't block this candidate's landing.
