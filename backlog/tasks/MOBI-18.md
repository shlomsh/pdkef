...
Superseded by SNG-10. The practice form is now the A4 "Employee details" form from the SNG-02 sketches,
generated by `scripts/generate-practice-form.mjs` from `scripts/practice-form-content.mjs`, with its
truth generated from the same layout. That covers this ticket's first half (a common page, common field
sizes, a score at or above 88.9% / 88.9%). Its content changed too, which this ticket had kept: the
next generation needed every kind on one form (text, combs, dates, checkboxes, a signature line and a
date line with no box). The benchmark half moved with the mobile editor into the SNG epic: MOBI-16 and
MOBI-17 are done, and SNG-05's surface and SNG-07's Simulator gate are measured on this form.
