...
- 2026-09-13: output name is `<first> + N more.pdf`, the PDF Title matches; `merged_` prefix retired. Owner decision (backlog/tasks/MERGE-03.md's own history, confirmed again in the follow-up review): the previous line above tried the tool-prefix shape a second time (`merged_<first>.pdf`, matching `signed_`/`redacted_`) and it did not stick - a merge's identity is the files it folded in, not a stamped tool name. `mergePlan.ts`'s `DEFAULT_OUTPUT_NAME_TEMPLATE` is now `'{name} + {count} more'` (was `'merged_{name}'`); `outputName` in `toolMessages.ts` carries the same shape in English and Hebrew (`'{name} + {count} more'` / `'{name} + {count} נוספים'`). `mergedTitle`/`mergedFileName` needed no logic change, since template substitution was already generic. Every unit fixture asserting `merged_...` updated (`mergePlan.test.ts`, `merge.test.js`, `PdfMergeTool.test.tsx`); three e2e specs still assert the old name (`merge-handoff.spec.js`, `merge-layout.spec.js`, `merge-mobile.spec.js`) and are owned by Shlomi's own pass, not this change.
- 2026-09-13, Shlomi's own pass (final reversal, this decision stands): back to `merged_<first file base name>.pdf`, no count, no connector, no template - the third and last swing of this pendulum. Every other tool marks its output with the tool's name on the source file's own name (`signed_`, `redacted_`, `-compressed`); `<first> + N more` read as a clever idea but broke that shape for no real gain, and the "+ N more" connector had no good answer for a locale whose script reads right to left (see MERGE-11's now-closed open question) or for a first file that already carries another tool's prefix. `mergePlan.ts` replaces `DEFAULT_OUTPUT_NAME_TEMPLATE` with a single literal constant, `MERGED_FILE_NAME_PREFIX = 'merged_'`, next to `mergedTitle`/`mergedFileName`, which now take only the first file's name - no `otherCount`, no `template` parameter, on either function. `outputName` is removed entirely from `toolMessages.ts` (both editions): there is no template left for a locale to phrase, so nothing needs localizing, exactly like Sign's literal `signed_${sourceFile.name}` in `PdfSignTool.tsx`. One file gets the same shape (`merged_x.pdf`); the old single-file special case (keep the name untouched) is gone. A first file that already carries a prefix chains rather than collapsing (`signed_form.pdf` -> `merged_signed_form.pdf`). The prefix is deliberately the same, un-translated, on `/he/merge/`, so files produced by either edition sort together in a folder - Sign does the same with `signed_`. Every fixture that asserted the `<first> + N more` shape is updated back (`mergePlan.test.ts`, `merge.test.js`, `PdfMergeTool.test.tsx`, `useMergeDraft.test.tsx`, `merge-handoff.spec.js`, `merge-layout.spec.js`, `merge-mobile.spec.js`, `merge-restore.spec.js`); the Hebrew `localizedTools` entry for `merge` (`src/content/localized-tools/he/merge.yaml`) needed its own step-3 copy and `sourceHash` re-published after an unrelated English copy fix (the `steps` field's "Open Options" line, now "Check Add page numbers", stale since MERGE-06/11 removed the Options disclosure) tripped the translation-freshness guard.
