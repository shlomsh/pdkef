...
- 2026-09-13, evening (Shlomi's review pass, second round): "Edit pages" is gone. On a phone a tap on a page reveals that page's own rotate / skip / open cluster and only that one (`data-selected` on the cell, phone-only CSS); a second tap, a tap on another page or outside the grid lets go. Three 44px buttons never share a row of 72px cells now, so nothing overlaps a neighbour. The "…" popover holds Add files, Clear all, Sort, Reset order (only while rearranged, the note wrapping inside the popover) and the Add page numbers checkbox, sized `min(100vw - 2rem, 22rem)` and end-anchored so it stays inside the viewport on both editions (it was cropped right, then left, before). The sheet is Download full width, then Share, Compress it and Sign it with 16px icons. Guards: merge-mobile, merge-direction-a, merge-share.
