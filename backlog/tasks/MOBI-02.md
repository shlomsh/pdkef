...
**Manual acceptance completed 2026-09-15.** The input fixture was
`output/pdf/mobi-02-acroform-check.pdf`: a one-page form with one canonical, live, empty text field
named `applicant.name`, one matching `/Widget`, and a valid appearance stream. Its page includes the
exact check steps and pass condition. After loading it in `/sign/`, placing a PDkef text element in
the field and exporting, the result was checked in Chrome and macOS Preview: the text remained
visible, clicking the former field produced no caret, and no editable control remained. A structural
inspection agreed: zero form fields and zero `/Widget` annotations in the export. Native AcroForm
editing is deliberately not part of this ticket; MOBI-02 closes as the safe flatten-on-export fix.
