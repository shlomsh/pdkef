...
2. **Validated content.** Preserve existing English YAML. Add `src/content/localized-pages/{locale}/{pageId}.yaml` through `src/content.config.ts`, reusing existing guide blocks. Use a separate typed collection for tool help where structure differs, with shared locale, reviewer, normalized `sourceHash`, reviewedAt, and publication fields. Require unique pairs and matching registry/content entries. Exclude drafts from public routes; keep samples in `docs/`.
...
- Product owner approves factual English source. The build calculates an `fnv1a64` `sourceHash` from normalized rendered English content; the translator records that value in the localized entry. It is a change fingerprint, not a security boundary. Do not use file modification dates or manually maintained version strings as freshness evidence.
...
- Engineer checks schema, static rendering, links, bidi, state safety, and page weight. A published entry whose `sourceHash` no longer matches its English page fails the build; drafts stay reviewable and carry a stale-preview indication. Only then does the content owner mark an entry published.
- Source edits flag translations stale per page. Safety/capability corrections block the affected page until reconciled; a cosmetic edit still needs a fresh source hash and reviewer decision before publication. Missing guides link explicitly to English. A missing article keeps that `(pageId, locale)` unpublished, not every reviewed page in the language.
