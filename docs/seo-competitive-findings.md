...
- **Release history** - in progress. `package.json` bumped 0.1.0 to 1.0.0 on `main` (`054b8db`,
  2026-09-11) and release notes drafted (nine tools, 20-language Sign, on-device drafts, Hebrew editions,
  CSP backstop). **The `v1.0.0` tag and the GitHub release are not yet created**: both `git tag` and
  `gh release create` were blocked by the agent sandbox's permission classifier, so Shlomi runs them
  (`git tag -a v1.0.0 054b8db && git push origin v1.0.0`, then `gh release create v1.0.0 --title
  "PDkef 1.0.0" --notes-file <notes>`). The awesome-selfhosted four-month clock starts from the tag
  date, not the version bump.
...
| Astro Showcase (astro.build/showcase) | Post the site URL as a comment in [withastro/roadmap#521](https://github.com/withastro/roadmap/discussions/521); no stated star/traction gate | Yes - genuinely built with Astro | Not yet posted |
