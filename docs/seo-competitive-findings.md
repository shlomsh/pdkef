...
| LOC-14 Indonesian target-size gate | open | 2026-09-12 | LOC-11's autocomplete sweep found the size-limit family densest in Indonesian (`kompres pdf 1 mb`, `mengecilkan pdf 200 kb`, `ukuran file lamaran kerja`), the intent `/pdf-wont-compress-to-100kb/` already serves in English; Trends, `hl=id` SERPs and GSC Indonesia run against the ROI gate before any page is written. | Shlomi's Trends, SERP and GSC screenshots (links in LOC-14) |
