 ...
 * its exclusion rule and its own prose notes. The two original outputs are unchanged: this file's
 * `practice-form` and `irs-1040` entries reproduce them byte-for-byte (verified by regenerating
 * and diffing against git history).
 *
...
/**
 * The practice form needs no exclusion pass (`excuse: null` - its nine widgets are all real
 * answer locations) and its own field names already say what they are for
 * (`parent_guardian_signature`, `signature_date`), so both its kind and its label come from the
 * field's own (inherited, not fully qualified) name rather than from Ff flags alone or a
 * caption. Reproduces `generate-practice-form-truth.mjs`'s original output exactly.
 */
const practiceFormName = ({ ctx, w, i }) => textOf(inherited(ctx, w, 'T')) ?? `field_${i}`;
...
const practiceForm = {
  ...
  render: null,
  excuse: null,
  kindOf: ({ ft, ff, maxLen, ...rest }) => {
    if (ft === '/Btn') return 'checkbox';
    if ((ff & COMB) && maxLen > 1) return 'comb';
    const name = practiceFormName({ ft, ff, maxLen, ...rest });
    // Order matters: `signature_date` is a date field that happens to carry the word signature,
    // so the more specific suffix test has to come first.
    if (/date$/i.test(name)) return 'date';
    if (/signature/i.test(name)) return 'signature';
    return 'text';
  },
  labelFor: practiceFormName,
  targetExtra: (target) => {
    target.notes = "from the form's own AcroForm widget: /Rect is the field, exact to the point";
  },
  notes: 'Self-labelling: every target is one of the form\'s nine AcroForm widgets, so the bounds are '
    + 'exact rather than annotated, and no eyeballing pass was needed. Generated by '
