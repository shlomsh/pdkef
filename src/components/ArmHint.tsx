 ...
 *
 * Portaled via Floating UI, the same pattern `Popover.tsx` already uses to get
...
export default function ArmHint({ tool, label, action, locked, autoShowTool, children }: {
  ...
  // like Popover.tsx's dropdowns but wrong here: it silently ate the Escape
  // press that PdfSignTool.tsx's/PdfRedactTool.tsx's own global handler uses to
  ...
  // Fullscreen re-scopes where a portal has to land, same as Popover.tsx: the
  ...
  return (
    <>
      {cloneElement(children as any, {
        ...
        ...getReferenceProps((children as any).props)
      })}
      {open && portalTarget &&
        createPortal(
          <div
