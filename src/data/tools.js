...
export const tools = [
  {
    ...
    gridDescription:
      'Add text, check marks, and your signature to a PDF on your phone or computer. Free, no signup, no upload.',
    sitemapPriority: '0.9',
    sitemapChangefreq: 'monthly',
    seoTitle: 'Sign PDF Online Free - Fill Forms, No Signup | PDkef',
    seoDescription:
    ...
    subhead:
      'Fill out and sign a PDF on your phone or computer, no printing or scanning. Add text, check marks, and a signature in 20 languages and scripts, or fill a printed row of boxes one letter per box. Free, open source, and your document stays on your device.',
    ...
    // one where a missing glyph can ruin the result.
    //
    ...
    // src/editor/text/fontCoverageReport.js here would pull it into RecentFiles'
    // client bundle (it imports this file) and cost real page weight for
    // data the card never renders directly. Instead
    // src/editor/text/languageCoverage.test.js is the seam: it checks every claim
    // here (family counts, which languages are "supported", the Hebrew+Latin
    // and Cyrillic-anchor specifics) against src/editor/text/fontCoverageReport.js
    // (W7, generated by scripts/generate-font-coverage-report.mjs from the
    // real font bytes via src/editor/text/fontCoverageTable.js) and fails the build
    // if this prose and the measured coverage disagree.
    //
    // This card is the product's loudest claim and is written that way. The
    // language support is the largest advantage in the tool that nothing else
    // free and browser-side matches (docs/seo-competitive-findings.md §4's
    // non-ranking table), and it had been buried below the fold in nine words.
    // So: lead with the count, say right-to-left is native, and let the
    // per-language rows carry the detail for anyone who wants it.
    //
    // `notYet` does NOT enumerate the scripts with no bundled font, and that
    // is a deliberate owner decision (2026-09-11), not an oversight to
    // helpfully fix. A list of gaps in the middle of the strongest claim on
    // the page reads as a disclaimer and dates badly - languages here are
    // added on request, so the list would be wrong the week someone asks. The
    // honest disclosure it replaces still happens, and happens somewhere
    // better: the editor names any character it cannot draw *while you type
    // it*, which is the FAQ entry below. languageCoverage.test.js pins that
      ...
      supported: [
        {
          ...
          note: 'All 25 text fonts and 13 handwriting faces are open to you for English, Spanish, French, German, Polish, Filipino, Bahasa Melayu, Gaeilge, or any other Latin-script language, whether you are filling a form or signing your name. Accented letters narrow the choice rather than remove it: 12 of the 25 text fonts and six of the 13 handwriting faces carry the full accented set, so a name with ł, š, ă or ż has fewer fonts to pick from than a name without. Vietnamese narrows it again and has its own line below. Whatever you type, the tool names any character it cannot draw while you are still typing, rather than letting it slip into the download.',
        },
        {
          native: '简体中文, 繁體中文 🇨🇳🇹🇼🇭🇰',
        ...
        },
        {
          native: 'Tiếng Việt 🇻🇳',
          name: 'Vietnamese',
          note: 'Fill a Vietnamese form and sign your name in Vietnamese, in Arimo, Tinos or Cousine, or sign in Mali, Sriracha, Mynerve or Amatic SC for a handwritten look. I checked these letter by letter against the real font files rather than assuming general Latin coverage carried over: every tone-and-diacritic vowel (ạ ả ấ ầ ẩ ẫ ậ ắ ằ ẳ ẵ ặ and the rest) is there, in all seven. Amatic SC draws capitals only, so a lowercase Vietnamese name comes out in small capitals rather than a cursive script. The other bundled fonts do not carry Vietnamese, so they are left off its list.',
        },
        {
          native: '한국어 🇰🇷',
          name: 'Korean',
        ...
        },
        {
          native: 'ไทย 🇹🇭',
          name: 'Thai',
          note: 'Fill a Thai form in IBM Plex Sans Thai, an upright text face, or sign your name in Mali or Sriracha, two handwriting faces with a different feel, Mali looped and rounded, Sriracha loopless and quicker, both doubling as a typed signature. Tone marks and the vowels that sit above and below the line land where they belong instead of drifting off the letter, in all three fonts.',
        },
        {
          native: 'پښتو 🇦🇫🇵🇰',
          name: 'Pashto',
        ...
        },
        {
          native: 'മലയാളം 🇮🇳🇦🇪',
          name: 'Malayalam',
          note: "Fill a Malayalam form and sign your name in Malayalam, in Anek Malayalam or Gayathri, a softer, more curved face, either of which also serves the large Malayali community across the UAE and the rest of the Gulf. Vowel signs that move before or wrap around the consonant they attach to are reordered and reshaped for the download, and chillu letters, the consonant-final forms unique to this script (word endings like ...ൻ or ...ൾ), draw and cluster correctly too, in either font. Worth saying why neither is the Noto face the other scripts here use: Noto Sans Malayalam breaks on reph, the RA-plus-virama pattern that spells an initial r before a consonant cluster, so I picked faces that draw it correctly instead. Both are set to modern, reformed spelling (the standard since 1971), not the older traditionally-ligated style. There is no handwriting-style Malayalam face yet, so a typed signature comes out upright rather than cursive.",
        },
        {
          native: 'Türkçe 🇹🇷',
          name: 'Turkish',
          note: "Fill a Turkish form and sign your name in Turkish. 12 text fonts carry the complete 29-letter alphabet, the same 12 that carry the full accented set in the Latin-script note above, plus Kalam, Mali, Sriracha, Tillana, Mynerve and Amatic SC if you want a handwritten look, any of which also works for a typed signature (Amatic SC draws capitals only, so a lowercase name comes out in small capitals). The three letters that set Turkish apart from plain accented Latin, Ğ, İ and Ş, are exactly what that general Latin-Ext sample does not check, so I verified them against the real font files rather than assuming coverage carried over.",
        },
      ],
    ...
    steps: [
      ...
      { title: 'Add text & signatures', text: 'Click to place text blocks, symbols, or signatures on the pages. If the form has a printed row of boxes, one per letter, tap it and your text lands one character per box.' },
    ...
    faq: [
      {
        question: 'How do I sign a PDF on my computer or phone?',
        answer: 'Open the PDF here in your browser. Add any text or check marks, then draw a signature, type one, or choose a signature image. Position it on the page and choose Download. You can use a mouse on a computer or a finger on a phone. No account or app installation is required.',
      },
      {
      ...
      {
        question: 'The form has a row of little boxes, one per letter. How do I fill those in?',
      ...
      },
      {
        ...
        answer: 'Yes. Russian, Ukrainian, and other Cyrillic text uses PT Sans for upright text, or Neucha or Amatic SC if you want a handwritten-feeling signature (Amatic SC draws capitals only, so lowercase comes out as small capitals, and neither carries the extra Kazakh letters). Thai has three choices: IBM Plex Sans Thai, an upright font for filling in forms, and two handwriting-style fonts you can also use for a typed signature, Mali and Sriracha, loopless where Mali is looped. All are bundled with the tool and checked against their real glyph coverage, so what you see on screen is what gets embedded in the file you download.',
      },
      {
...

  {
    ...
    faq: [
      ...
      { question: 'Can I convert just a few pages instead of the whole PDF?', answer: 'Yes. Enter a page range in the page selector: specific pages separated by commas (1, 3, 5), closed ranges (2-4), or open-ended ranges like "8-" (page 8 to the end) or "-4" (the start through page 4). Only those pages convert. Useful for pulling one diagram or photo out of a long scanned document without converting the rest.' },
...

  {
    ...
    faq: [
      ...
      { question: 'Can I mix JPG and PNG images in the same PDF?', answer: "Yes. Add any combination of JPG and PNG files and they combine into one PDF together, each embedded in its own format - there's no need to convert everything to one type first." },
      { question: 'Why does each page come out a different size?', answer: 'Every PDF page is sized to match its image exactly, one image pixel to one PDF point, so a portrait photo becomes a portrait page and a wide screenshot becomes a wide page. Nothing is cropped, stretched, or letterboxed to fit a standard paper size.' },
    ],
  },
...

  {
    ...
    faqIconPos: 'tr',
    steps: [
      { title: 'Open your PDF', text: 'Select or drag in the file you want to compress. It stays on your device.' },
