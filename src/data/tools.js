...
export const tools = [
  {
    ...
    // if this prose and the measured coverage disagree. It checks the other
    // direction too (SEO-07): every script `notYet` names as having no
    // bundled font is verified against FONT_COVERAGE to really have none, so
    // a font landing that draws Gujarati fails the build rather than leaving
    // this card understating the tool.
    //
    ...
    languages: {
      ...
      lead:
        "Conjunct letters, vowel signs, ligatures and right-to-left text are shaped for the download the same way they look on screen, so what you type is exactly what lands in your file. A right-to-left box grows leftward from a fixed right edge, the way Hebrew and Arabic actually behave. And if a character cannot be drawn in any bundled font, the editor names it while you are still typing, rather than leaving an empty box for you to find after you download. Tap a language below for the specifics.",
      ...
      notYet:
        "Some scripts have no bundled font here at all, and it is better to read that than to discover it: Gujarati, Kannada, Odia, Sinhala, Khmer, Lao, Burmese, Amharic, Armenian, Georgian, and emoji. Type one of those and the editor names the character it cannot draw while you are still typing, so nothing quietly becomes an empty box in the file you download. I add languages when someone actually asks for one, rather than guessing ahead of demand, so the fastest way to get yours prioritized is to ask below.",
    ...
    faq: [
      ...
      {
        question: 'What happens if the tool cannot draw a character I type?',
        answer: "It tells you while you are typing it, and names the character. That is the guarantee worth knowing before you fill in a whole form: the fonts are bundled with the tool and checked against the glyphs actually inside the font files, so a character no bundled font can draw is flagged in the editor rather than turning into an empty rectangle in the PDF after you download. If the font you picked cannot draw your script but another bundled one can, the tool switches to that one and tells you it did. Gujarati, Kannada, Odia, Sinhala, Khmer, Lao, Burmese, Amharic, Armenian, Georgian and emoji have no bundled font here at all yet; every language listed in the language section above does.",
      },
      {
