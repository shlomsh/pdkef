...
const sitePath = z
  ...
  .regex(
    /^\/[a-z0-9]+(?:-[a-z0-9]+)*\/$/,
    'must be a lower-case site path with a trailing slash, e.g. "/sign/"',
  );
...
const contentPages = defineCollection({
  ...
  schema: z.strictObject({
    ...
    // The FAQ is not optional and not a stub: it is rendered on the page AND
    // mirrored into FAQPage JSON-LD, and Google wants a real one. Four is the
    // floor; every page today has six. A missing `answer` is now a build
    // error, which is the single thing this migration was asked to buy.
    faq: z
      .array(z.strictObject({ question: plain(15, 120), answer: plain(40, 600) }))
      .min(4)
      .max(12),
  }),
});

export const collections = { contentPages };
