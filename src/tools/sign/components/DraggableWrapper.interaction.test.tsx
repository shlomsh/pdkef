...
// growth, dark mode, mobile full-width toolbar (see SignToolbar.test.tsx for
...
describe('DraggableWrapper interaction/visual states (E1.4)', () => {
  ...
  describe('floating toolbar visibility + stable top placement', () => {
    ...
    it('keeps Floating UI placement stable above the element and does not configure a bottom fallback', () => {
      ...
      // array DraggableWrapper.tsx builds and hands to useFloating (captured
      ...
      // The FIRST call reflects what DraggableWrapper.tsx actually asked
  ...
  describe('RTL toolbar alignment + leftward growth', () => {
    it('requests placement `top-end` (right-aligned) for RTL text and `top-start` (left-aligned) for LTR text', () => {
      ...
      // this is the literal `placement` value DraggableWrapper.tsx computes
      ...
      // DraggableWrapper.tsx actually requested (see the mock's comment above).
    ...
    it('uses RTL placement for an empty box seeded RTL by its document (SIGN-34)', () => {
    ...
    it('never applies a `--rtl` toolbar class — horizontal alignment is driven entirely by Floating UI placement', () => {
      ...
      // DraggableWrapper.tsx's comment claiming it drove horizontal alignment
  ...
  describe('whiteout bounds', () => {
    ...
    it('clamps the left/top position when resizing from the left/top handle so the box stays fully on the page', () => {
      // DraggableWrapper.tsx's handleResizeMove clamps WIDTH/HEIGHT to
    ...
    // post-mortem — see DraggableWrapper.gestureInvariants.test.tsx for the
