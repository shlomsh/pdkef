...
**Shipped.** The ten current `src/content/content-pages/` entries are live, registered in
`src/data/contentPages.js` with a hub, sitemap priority and changefreq, and cross-linked.

- **Long-tail landing pages.** `/sign-pdf-no-signup/`, `/open-source-pdf-editor/`,
  `/install-pdf-app/`, `/offline-pdf-form-filler/`, `/blur-vs-blackout-vs-delete-pdf/`, and
  `/permanently-delete-text-from-pdf/` are all live.
