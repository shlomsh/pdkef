...
| ARCH-07 | P3 | Open | **Stand up `src/i18n/` ahead of the documentation-localization build.** `docs/app-documentation-localization-plan.md` (2026-08-28) recommends a documentation-language selector and statically generated translated routes (`/he/how-to-sign-a-pdf-on-android/`, etc.); none of that code exists yet. Create `src/i18n/` now — locale list/resolution (parsing a `/xx/` path prefix against supported documentation locales), a message-catalog shape for the selector's own UI strings, and formatting helpers (native language-name display, already partially duplicated between `ToolLanguagesCard.astro`'s language list and `documentationLocales.ts`) — so the localization build lands directly into the right module instead of being retrofitted out of `contentPages.js`/`[contentPage].astro` later. Per the boundary, `i18n` must not depend on document mutation: it's routing/copy for the static SEO shell (§1.1's "sacred" invariant), never the PDF editor's own state. Scope is the module scaffold only, not executing the localization plan itself. **Status correction 2026-08-29: "none of that code exists yet" is no longer true, and the scaffold has to fit what already shipped.** `src/pages/[locale]/[contentPage].astro`, `src/content/localized-pages/he/` (8 pages), `src/lib/documentation.ts`, `src/data/documentationLocales.ts`, `src/data/documentationCountries.ts`, `DocumentationLanguageSelector.astro` and `DocumentationCoverage.astro` all landed in `b4ffd96`. **All 8 Hebrew pages carry `status: 'draft'`, and drafts are gated behind `PDKEF_DOCS_PREVIEW=1`** (`isDocumentationPreview` in `documentation.ts`), so a normal `npm run build` emits 23 pages, no `/he/` routes, and a sitemap unchanged at 20 URLs - verified on that commit. With the flag set the build emits 30 pages including all 8 Hebrew ones, each `noindex, follow` with `lang="he" dir="rtl"` and a self-canonical, and still absent from the sitemap. So the SEO surface is safe today, and ARCH-07 is now a *consolidation* job: `src/i18n/` must absorb the locale resolution already living in `documentationLocales.ts` and `documentation.ts` rather than being stood up beside it, or there will be two locale registries instead of the one duplicate this ticket set out to remove. |
...
| --- | --- | --- | --- |
| FONT-01 | P1 / release blocker (rides SIGN-19) | Open | **Recalibrate the export-render guard's cross-platform tolerance.** `exportRenderHarness.js`'s `MIN_TOLERANCE_PCT = 12.5` was never measured across machines; the guard is red on macOS at HEAD with zero local changes (`latin-caveat` 13.68%, `latin-great-vibes` 17.61%). Same root cause as SIGN-19 - a rasteriser gap between macOS dev machines and the `ubuntu-latest` CI runner, found independently while adding the Japanese/Bengali corpus cases. **Do not resolve separately: extend whatever calibration policy SIGN-19 lands (per-platform floors, one pinned rasterising platform, or CI-only execution) to this harness too**, rather than picking a second, possibly conflicting policy. Depends on SIGN-19's decision landing first. |
...
| FONT-03 | P2 | Open | **Malayalam.** Zero coverage; one clean, unscreened candidate on record (Noto Sans Malayalam, OFL). Needs the pixel-diff corpus-guard treatment (Devanagari/Bengali model: conjuncts + vowel reordering), a deliberate traditional-vs-reformed-orthography call, and the usual catalogue/copy wiring. Screen the candidate against a generated fontkit corpus before committing to it, per the Gurmukhi/Telugu lesson below. Candidate research rules: [docs/font-candidate-research-brief.md](./docs/font-candidate-research-brief.md). |
...
| FONT-08 | P3 | Open | **Second-font / missing-style research across every single-font script.** Two distinct gaps: **(a) no upright/text-style option at all** - ~~Devanagari and Thai each have exactly one bundled face (Kalam, Mali) and both are handwriting~~ **Thai's half landed 2026-08-29 (FONT-08a): IBM Plex Sans Thai added as an upright text face alongside Mali.** Screened against Sarabun (top-ranked candidate) and Kanit, both of which measurably failed the fontkit-vs-browser advance-parity check (Guard A) on ordinary Thai words - Sarabun by 1.4-3.0% of string width, Kanit by a smaller but still real 0.3-1.0% - despite neither carrying `calt`. IBM Plex Sans Thai does carry `calt` (flagged and stress-tested specifically against the classic Thai tall-consonant/tone-mark collision case, ปั๊กฝ้ายให้ฟังกิ๊บ) and passed Guard A cleanly (0.05px unhinted tolerance) on every sample including that stress case, so it shipped despite the flag rather than being screened out by it - see `e2e/sign/thai-font-parity.spec.js`. **Devanagari's half remains open**: Kalam is still the only bundled Devanagari face and is handwriting-only, so picking an upright font for Hindi/Marathi text still resolves the whole element to a handwritten look. **(b) single-font scripts with no second choice for variety** - Bengali, Punjabi/Gurmukhi, Telugu, Tamil, the Arabic family (Scheherazade New), Japanese, Chinese SC/TC, Korean, plus Cyrillic and Greek (both text-only today, no handwriting option either), and now also Thai's *handwriting* side (Mali is still the only Thai handwriting face). Named-but-unscreened candidates already on record: Sriracha (2nd Thai handwriting, same-day runner-up to Mali), a 2nd Cyrillic face, a 2nd Hebrew handwriting face, more Latin handwriting styles. **Research rules and the exact current catalogue to screen against: `docs/font-candidate-research-brief.md`, referenced here and by FONT-03 but not actually present in the repo as of this entry** - the research for FONT-08a was done directly against `fonts.js`/`fontCoverage.test.js`/TODO.md's own Alef/PT Sans/Mali precedent instead; re-create the brief or fix the link before relying on it again. |
