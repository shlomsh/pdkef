...
const EPICS = [
  ...
  { id: "E9", name: "Offline-first app shell", goal: "Open the fully cached application without a reachable server, while keeping draft retention policy-driven." }
];
...
const TICKETS = [
  ...
  { id: "E6", epic: "E6", lane: "X", status: "todo", priority: "normal", group: "Editor / UX polish",
    title: "Live in-place preview for Redact's Delete tool (stop rendering, not decorate over)",
    dependsOn: [],
    acceptance: "A marked object currently gets a CSS overlay (border, previously a hatch fill) on top of the untouched pdf.js-rendered canvas underneath - decoration, not removal. The ask is for the object to genuinely not render at all while marked, matching what Download actually produces. Design (discussed 2026-08-14, not started): on every mark/unmark, regenerate just the affected page's bytes by running deleteObjectsFromPdf() (src/lib/deleteObjects.js) against the source with that page's currently-marked deletions, then re-render that page's pdf.js source from the regenerated bytes instead of the original; every other page keeps rendering from the untouched document. This reuses the exact export code path as the live-preview generator, so preview and download can't drift apart, unlike a separate preview renderer would. Needs: (1) an async request-generation guard so a stale regeneration triggered by rapid mark/unmark can't clobber a newer one, same loadIdRef pattern PdfRedactTool.jsx already uses for load races; (2) PdfPageCanvas needs to accept a per-page document/source override instead of always rendering every page from the one shared pdfDocument, so only the affected page's canvas swaps; (3) scoped to type:'delete' elements only - blackout/blur/whiteout boxes already preview accurately and cheaply via their existing overlay div (a solid color really does look like the eventual flattened result; delete is the one mode where looking right requires the object to actually be absent, not covered)." }
  ...
  // ----- E9 - offline-first app shell -----
  ,{ id: "E9.1", epic: "E9", lane: "B", status: "done", priority: "high",
    title: "Offline-first app shell and policy-driven draft expiry",
    dependsOn: [],
    acceptance: "Done, built across two overlapping efforts that converged in the same tree rather than one linear pass - the homepage resume surface first, the shared retention policy and cache-first shell layered on afterward. draftPolicy.js is now the one place MAX_AGE_MS, createDraftRetention and isDraftExpired live; draftStore.js re-exports MAX_AGE_MS rather than redeclaring it, and both the IndexedDB read path (loadDraft) and the synchronous localStorage read path (readDraftMeta, new) call isDraftExpired instead of hand-rolling the Date.now() comparison - a stale localStorage draft-meta hint (has-draft:<tool> plus a sibling draft-meta:<tool> key holding fileName/savedAt/preview) is now cleared the moment it's read past MAX_AGE_MS, not just when IndexedDB itself is next opened. readDraftMeta backs a new ResumeDraftCard.jsx on the homepage - 'pick up where you left off', most-recently-saved draft first, page-1 JPEG preview (thumbnails.js's new renderDraftPreview, 96px/quality 0.7, rendered once per loaded file in useDraftPersistence.js and carried on every autosave) in place of a generic icon, no dismiss control by design since hiding the card would imply the draft's bytes were gone when they aren't. FileDropzone.jsx (the homepage launcher) moved from client:load to client:only='preact': it depends on localStorage, which does not exist during Astro's Node-side SSR pass, and client:only sidesteps the SSR/hydration mismatch entirely rather than accepting a post-hydration content swap. The now-secondary 'or start something new' dropzone compacts on desktop once a draft exists (icon and privacy line dropped, single row) via a new homepage-only FileDropzone.module.css - deliberately not added to the shared Dropzone.module.css, which BasePdfTool.jsx also imports and Astro server-renders on every tool page, so any CSS added there is inlined into all of them regardless of whether a given page ever sets the compact prop; the per-page inline CSS budget (check-css-bundle.js) was already at 79755/80000 bytes before this shipped. Separately, the service worker (public/sw.js) moved from network-first navigations to cache-first-with-background-refresh: install now fetches scripts/generate-precache-manifest.mjs's build-time output (dist/precache-manifest.json, every non-manifest/non-sw.js file in dist, walked and written post-build - wired into `npm run build`) and populates one atomic cache keyed by a content-hashed CACHE_VERSION (sw.js ships with a __BUILD_ID__ placeholder the same script replaces after hashing the manifest), so a returning visitor gets the last successful install's complete shell even with the server unreachable; navigations key on pathname (navigationCacheKey), not the full URL, so a query-string route like /sign/?action=open hits the same cached entry as /sign/ while location.search stays available to the hydrated client. Fixed in passing: sw.js's manifest and per-URL fetches used bare relative path strings ('/precache-manifest.json', each manifest entry) relying on the implicit base-URL resolution a real ServiceWorkerGlobalScope provides - correct in production, but serviceWorker.test.js's Node vm.runInNewContext sandbox has no such implicit base, so `new Request('/precache-manifest.json')` threw ERR_INVALID_URL there. Added a small resolve() helper that resolves every fetch explicitly against self.location.origin (already load-bearing elsewhere in the file for the fetch handler's own origin check), which behaves identically in both environments; cache keys stay bare pathnames, unchanged, since navigationCacheKey's lookup convention depends on that. Verified: production build clean, all four CSS guards pass against it (79633/80000 bytes, under the pre-existing baseline), 533/533 unit tests (up from 449), 0 typecheck errors, 0 warnings, 27 hints." }
];
